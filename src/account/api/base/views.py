...
class LoginWithGoogleView(generics.GenericAPIView):
    """
    Login or register a user using google oauth2 token.
    """

    permission_classes = []
    serializer_class = serializers.LoginGoogleSerializer
...
class LoginWithGithubView(LoginWithGoogleView):
    """
    Login or register a user using github oauth2 token.
    """

    serializer_class = serializers.LoginGithubSerializer


class LoginWithTwitterView(LoginWithGoogleView):
    """
    Login or register a user using twitter oauth2 token.
    """

    serializer_class = serializers.LoginTwitterSerializer
...
class RegisterAPIView(generics.CreateAPIView):
    ...
    @swagger_auto_schema(responses={201: serializers.UserSerializer})
    def post(self, request, *args, **kwargs):
...
class LoginAPIView(generics.GenericAPIView):
    ...
    def post(self, request):
        ...
        if user.verified_email:
            user_details = serializers.UserSerializer(user).data
            response_data = {"tokens": get_tokens_for_user(user), "user": user_details}
            return Response(data=response_data)
...
class ValidateForgetPasswordOtpView(generics.GenericAPIView):
    ...
    @swagger_auto_schema(responses={200: serializers.ForgetPasswordTokenSerializer})
    def post(self, request, *args, **kwargs):
        serializer = self.serializer_class(data=request.data)
        serializer.is_valid(raise_exception=True)
        ...
        return Response(
            data={"uidb64": uidb64, "token": token}, status=status.HTTP_201_CREATED
        )
